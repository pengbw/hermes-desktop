    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      ...
      >
        ...
        <div className="px-5 py-4 flex flex-col gap-4 overflow-y-auto">
          ...
          <div>
            <label className="text-[12px] font-medium text-muted-foreground mb-1 block">{t("cron.name")}</label>
            <input
          ...
          <div>
            <label className="text-[12px] font-medium text-muted-foreground mb-1 block">{t("cron.prompt")}</label>
            <textarea
          ...
          <div>
            <label className="text-[12px] font-medium text-muted-foreground mb-1 block">{t("cron.schedule")}</label>
            <input
          ...
          <div>
            <label className="text-[12px] font-medium text-muted-foreground mb-1 block">{t("cron.skills")}</label>
            <div className="flex flex-wrap gap-1 mb-2">
              {skills.map((skill) => (
                <span key={skill} className="text-[11px] px-2 py-0.5 rounded-full bg-primary/10 text-primary flex items-center gap-1">
                  {skill}
                  <button className="bg-transparent border-0 text-primary cursor-pointer text-[10px] p-0" onClick={() => handleRemoveSkill(skill)}>×</button>
                </span>
              ))}
            </div>
              ...
              {showSkillDropdown && filteredSkills.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg z-50 max-h-[180px] overflow-y-auto">
                  {filteredSkills.map((skill) => (
                    ...
                    >
                      <span className="text-[11px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground shrink-0">{skill.category}</span>
                      <span className="truncate">{skill.name}</span>
                    </button>
                  ))}
                </div>
              )}
              {showSkillDropdown && skillInput && filteredSkills.length === 0 && installedSkills.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-card border border-border rounded-lg shadow-lg z-50 p-3 text-[12px] text-muted-foreground text-center">
                  {t("cron.noSkillMatch")}
                </div>
              )}
            </div>
...
function CronOutputModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={onClose}>
      <div
        className="bg-card rounded-xl border border-border shadow-xl w-[600px] max-h-[80vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between h-10 px-3 border-b border-border">
          <h3 className="text-base font-semibold text-foreground m-0">
        ...
        <div className="px-5 py-4 overflow-y-auto">
          {loading ? (
            <div className="text-center py-8 text-muted-foreground text-sm">{t("cron.loading")}</div>
          ) : outputs.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground text-sm">{t("cron.noOutputs")}</div>
          ) : (
            <div className="flex flex-col gap-3">
              {outputs.map((output) => (
                <div key={output.id} className="rounded-lg border border-border bg-muted/30 p-3">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    {output.started_at && (
                      <span className="text-[10px] text-muted-foreground">{output.started_at}</span>
                    )}
                    {output.duration != null && (
                      <span className="text-[10px] text-muted-foreground">{output.duration}ms</span>
                    )}
                  </div>
                  <pre className="text-[11px] text-foreground whitespace-pre-wrap break-all m-0 max-h-[150px] overflow-y-auto">
                    {output.output}
                  </pre>
                </div>
              ))}
...
export default function CronSettings({ t }: CronSettingsProps) {
  ...
  const handleSave = async (data: { name: string; prompt: string; schedule: string; skills: string[] }) => {
    try {
      if (editingJob) {
        await TauriCommands.cronUpdateJob(editingJob.id, data.name, data.prompt, data.schedule, data.skills);
      } else {
  ...
  const handleTrigger = async (id: string) => {
    try {
      await TauriCommands.cronTriggerJob(id);
    } catch (e) {
      alert(String(e));
    }
  };
            ...
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[11px] text-muted-foreground mt-0.5 truncate">
                  ...
                  {job.skills.length > 0 && (
                    <span className="ml-2">{t("cron.skillsLabel")}: {job.skills.join(", ")}</span>
                  )}
                </div>
                {job.next_run && (
