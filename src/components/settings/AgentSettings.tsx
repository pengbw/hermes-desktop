    ...
    <div className="animate-in fade-in slide-in-from-bottom-1.5 duration-200">
      ...
      {config && (
        <div className="flex items-center gap-2 px-3 py-2 bg-muted rounded-lg text-xs mb-3">
          <span className="text-muted-foreground font-medium shrink-0">{t("settings.configPath")}:</span>
      ...
      <div className="bg-card rounded-xl p-5 shadow-sm mt-3">
        <h3 className="text-[15px] font-semibold text-foreground m-0 mb-4">{t("agent.sectionTitle")}</h3>
        <div className="flex flex-col gap-3.5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex items-center gap-2">
              ...
              {!modelListLoading && !provider && (
                <span className="text-xs text-muted-foreground whitespace-nowrap">{t("agent.selectProviderFirst")}</span>
              )}
              {modelListError && (
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] text-muted-foreground font-medium">{t("agent.maxTurns")}</label>
            <div className="flex items-center gap-3.5">
              ...
              <span className="min-w-[32px] h-8 flex items-center justify-center bg-primary text-primary-foreground rounded-lg text-sm font-bold shrink-0">{maxTurns}</span>
            </div>
            {dirtyFields.has("maxTurns") && (
              <span className="text-[10px] px-1.5 py-px rounded-md bg-primary/20 text-primary font-semibold w-fit">{t("common.modified")}</span>
            )}
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] text-muted-foreground font-medium flex items-center gap-2">
            ...
            <p className="text-xs text-muted-foreground m-0">
              {t("agent.hermesApiBaseHint") || "Hermes Agent 本地网关的 API 地址，工作室角色对话通过此网关路由"}
            </p>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] text-muted-foreground font-medium flex items-center gap-2">
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[13px] text-muted-foreground font-medium">{t("system.workspace.rootDir")}</label>
            <input
              type="text"
              className="w-full px-3 py-2 border border-input rounded-lg text-sm bg-background text-foreground outline-none focus:border-primary transition-colors h-10"
            ...
            />
            <p className="text-xs text-muted-foreground m-0">{t("system.workspace.rootDirHint")}</p>
          </div>
        </div>
